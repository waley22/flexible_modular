        ...
        for mobile_mode in ["Mob", "Fix"]
            ...
            for flex_idx1 in 0:0.1:1
                for flex_idx2 in 0:0.1:1
                    ...
                    @printf("%s set1+ %s: flex_idx1: %.1f, flex_idx2: %.1f\n", mobile_mode, dataset, flex_idx1, flex_idx2)
                    ...
                    while true
                        ...
                        u_star_dict[iter], second_obj = oracleIR(false, opt, x_val, x_d_val, F, h, G, G_d, B2, B2_d, d, B1, B1_d, E, c2, c2_d)
                    ...
                    if write_to_csv
                        if !isdir("./data/$(setName)/$(dataset)")
                            mkdir("./data/$(setName)/$(dataset)") 
                        end
                        if !isdir("./data/$(setName)/$(dataset)/Variable_result")
                            mkdir("./data/$(setName)/$(dataset)/Variable_result")
                        end
                        ...
                        CSV.write("./data/$(setName)/$(dataset)/Variable_result/Set1_result_x_design_$(mobile_mode)_FI1=$(flex_idx1)_FI2=$(flex_idx2).csv", Containers.rowtable(x_d_out; header = [:i, :m, :value]))
                        CSV.write("./data/$(setName)/$(dataset)/Variable_result/Set1_result_y_flow_$(mobile_mode)_FI1=$(flex_idx1)_FI2=$(flex_idx2).csv", Containers.rowtable(y_out; header = [:i, :j, :l, :t, :value]))
                        CSV.write("./data/$(setName)/$(dataset)/Variable_result/Set1_result_y_config_$(mobile_mode)_FI1=$(flex_idx1)_FI2=$(flex_idx2).csv", Containers.rowtable(y_d_out; header = [:i, :l, :t, :value]))
                        CSV.write("./data/$(setName)/$(dataset)/Variable_result/Set1_result_uncertain_j_list_$(mobile_mode)_FI1=$(flex_idx1)_FI2=$(flex_idx2).csv", Containers.rowtable(uj_out; header = [:uj, :t, :value]))
                        CSV.write("./data/$(setName)/$(dataset)/Variable_result/Set1_result_worst_case_$(mobile_mode)_FI1=$(flex_idx1)_FI2=$(flex_idx2).csv", Containers.rowtable(u_out; header = [:uj, :t, :value]))
                    end
